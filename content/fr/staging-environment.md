...
The staging environment uses the same rate limits as {{<link "described for the production environment" ">}} with the following exceptions:
...
The staging environment submits pre-certificates to the Let's Encrypt {{<link "Testflume" ">}} and Google [testtube](http://www.certificate-transparency.org/known-logs#TOC-Test-Logs) CT test logs and includes returned SCTs in the issued certificates.
